...
import com.mega.browser.mobile.android.DeviceCapabilities
...
class PrivacySettingsFragment : AbstractSettingsFragment() {
    ...
    private fun updatePasswordChoice(choice: PasswordChoice, activity: Activity, summaryUpdater: SummaryUpdater) {
        if (choice == PasswordChoice.CUSTOM) {
            ...
            val prefs: SharedPreferences = activity.getSharedPreferences("com.mega.browser.mobile.android", Context.MODE_PRIVATE)
    ...
    private fun clearHistory(): Completable = Completable.fromAction {
        ...
        if (activity != null) {
            ...
            com.mega.browser.mobile.android.utils.WebUtils.clearHistory(activity, historyRepository, databaseScheduler)
        } else {
    ...
    private fun clearCookies(): Completable = Completable.fromAction {
        val activity = activity
        if (activity != null) {
            com.mega.browser.mobile.android.utils.WebUtils.clearCookies(activity)
        } else {
    ...
    private fun clearWebStorage() {
        ...
        context?.let { com.mega.browser.mobile.android.utils.WebUtils.eraseWebStorage(it) }
