...
import com.mega.browser.mobile.android.search.Suggestions
...
class GeneralSettingsFragment : AbstractSettingsFragment() {
    ...
    private fun updateProxyChoice(choice: ProxyChoice, activity: Activity, summaryUpdater: SummaryUpdater) {
        val sanitizedChoice = com.mega.browser.mobile.android.utils.ProxyUtils.sanitizeProxyChoice(choice, activity)
    ...
    private fun showDownloadLocationDialog(summaryUpdater: SummaryUpdater) {
        BrowserDialog.showCustomDialog(activity) {
            ...
            setSingleChoiceItems(R.array.download_folder, n) { _, which ->
                when (which) {
                    0 -> {
                        userPreferences.downloadDirectory = com.mega.browser.mobile.android.utils.FileUtils.DEFAULT_DOWNLOAD_PATH
                        summaryUpdater.updateSummary(com.mega.browser.mobile.android.utils.FileUtils.DEFAULT_DOWNLOAD_PATH)
                    }
                    1 -> {
    ...
    private fun showCustomDownloadLocationPicker(summaryUpdater: SummaryUpdater) {
        activity?.let { activity ->
            ...
            val regularColor = com.mega.browser.mobile.android.utils.ThemeUtils.getTextColor(activity)
            ...
            BrowserDialog.showCustomDialog(activity) {
                ...
                setPositiveButton(R.string.action_ok) { _, _ ->
                    ...
                    text = com.mega.browser.mobile.android.utils.FileUtils.addNecessarySlashes(text)
    ...
    private class DownloadLocationTextWatcher(
        ...
        override fun afterTextChanged(s: Editable) {
            if (!com.mega.browser.mobile.android.utils.FileUtils.isWriteAccessAvailable(s.toString())) {
